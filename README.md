...
Hardware interrupts are generated by hardware independent of what is being run in the computer. They go through the PIC (Programmable Interrupt Controller) and then to the CPU. While software interrupts start at the CPU. Both then get directed to their IDT entries in the CPU which is used to jump to the respective handlers.

All interrupts run at kernel level, the interrupt handler changes privilege level.

## .x Initialization
There are two PICS, the Master (IRQ0–IRQ7) and Slave (IRQ8–IRQ15). They need to be mapped to the IDTs.
...
- A few interrupts do not need an IDT entry, but they do not need to be configured in any way.
...
## .x Interrupt list
There may be a total of 256 entries, one for each interrupt number. Only a few of them are needed in this OS, and so only those are configured.

List of configured interrupts:

| Number | Name | DPL | Category | Notes |
| ----- | ---- | --- | --- | --- |
| 0 | Divide Error | 0 | Exception | Generated by CPU. |
| 6 | Invalid Opcode | 0 | Exception | Generated by CPU. |
| 8 | Double Fault | 0 | Exception | Generated by CPU. |
| 13 | General Protection Fault | 0 | Exception | Generated by CPU. |
| 14 | Page Fault | 0 | Exception | Generated by CPU. |
| 32 | Timer | 0 | Hardware | IRQ0. For preemptive scheduling. |
| 33 | Keyboard | 0 | Hardware | IRQ1. |
| 0x80 | System Call | 3 | Software | Called from software. |

Exceptions are generated by the CPU.
...
Once interrupt 0x80 is run, it switches to kernel mode in the CPU before reaching the handler, and after going through the interrupt handler, goes to the entry for system calls, selecting the right system call from the list containing all of them, executing it, and returning.
