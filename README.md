...
## .x Initialization
There are two PICS, the Master (IRQ0–IRQ7) and Slave (IRQ8–IRQ15). Each IRQ line needs to be mapped to its IDT Entry.
...
## .x Interrupt List
There are 256 possible IDT entries, one for each interrupt number. Only a few of them are needed in this OS, and so only those are configured.
...
| 13 | General Protection Fault | 0 | Exception | |
| 14 | Page Fault | Exception | |
| 32 | Timer | Hardware | IRQ0. For preemptive scheduling. |
| 33 | Keyboard | Hardware | IRQ1. |
| 0x80 | System Call | Software | Called from software. |
